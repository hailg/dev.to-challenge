...
import { productDao, productLinkDao, userDao } from '@/backend/dao/dao';
...
export async function GET(request, { params }) {
  ...
  const productLink = await productLinkDao.get(slug, 'ProductLink');
  if (!productLink) {
    return new Response(null, {
      status: 404
    });
  ...
  if (now > productLink.expiresAt) {
    return new Response(null, {
      status: 404
    });
  }
  const product = await productDao.get(productLink.productId, 'Product');
  if (!product) {
    return new Response(null, {
      status: 404
    });
